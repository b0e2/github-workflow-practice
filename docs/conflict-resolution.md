...
## 충돌 기록 #3: CONTRIBUTING 가이드 섹션 이어붙이기 충돌

### 1. 기본 정보

- 발생 날짜: 2026-09-18
- 충돌 파일: `docs/CONTRIBUTING.md`
- 작업 브랜치: `feature/juseong-contributing-prohibitions`
- PR 대상 브랜치: `main`
- 충돌 해결자: 정빈 (`@b0e2`)
- 브랜치 작성자: 이주성 (`@YiJuseong`)
- 관련 PR: [PR #20](https://github.com/gitflow-practice-team/github-workflow-practice/pull/20)

### 2. 충돌 전 커밋

- 공통 기준 커밋: `6a2f164` (`Merge pull request #14`)
- feature 브랜치 커밋: `afe9eb25db62c887f9713585edb9ea42bcf40c04`
- 병합한 최신 main: `277b14d3f5318d372a188393e2d69b9c20bd0c52`
- 해결 커밋: `9e1d89b3892029c31c8ae75d81de94654ba3574f`

### 3. 상황

PR #20 브랜치는 `docs/CONTRIBUTING.md` 끝에 `## 13. 금지 사항`, `## 14. 기본 협업 흐름`, `## 15. 관련 문서`를 추가했다.

그 사이 `main`에는 PR #19가 병합되면서 같은 파일 끝에 `## 10. 충돌 발생 시 대응 흐름`, `## 11. 충돌 해결 시 주의사항`, `## 12. Git 트러블슈팅 원칙`이 추가됐다.

양쪽 모두 같은 마지막 줄 뒤에 서로 다른 섹션 블록을 이어붙였기 때문에, Git이 두 블록의 순서를 결정하지 못해 충돌이 발생했다.
...
`git status --short`에서 `docs/CONTRIBUTING.md`가 `UU` 상태로 표시됐다.
...
- `HEAD`: PR #20 브랜치가 추가한 섹션 13~15
- `277b14d`: `main`에 먼저 병합된 섹션 10~12

### 6. 비자명 충돌로 판단한 이유

한쪽만 선택하면 다음 문제가 생긴다.

- feature 변경만 유지하면 충돌 대응 흐름과 트러블슈팅 원칙 가이드가 사라진다.
- main 변경만 유지하면 금지 사항과 기본 협업 흐름 가이드가 사라진다.
- 두 블록을 그대로 이어붙이면 섹션 번호가 13, 14, 15, 10, 11, 12 순서로 뒤섞여 문서 목차가 깨진다.

즉 텍스트를 합치는 것으로 끝나지 않고, 문서 구조와 번호 체계를 이해해야 해결할 수 있는 충돌이다.

### 7. 해결 전략

양쪽 내용을 모두 유지하되 섹션 번호 순서대로 재배치하는 방식을 선택했다.

1. `main`의 섹션 10~12를 앞에 배치했다.
2. PR #20의 섹션 13~15를 그 뒤에 배치했다.
3. 섹션 구분선과 빈 줄 형식을 기존 문서 규칙에 맞췄다.
4. 충돌 마커를 모두 제거했다.
5. 섹션 12의 트러블슈팅 설명과 섹션 13의 금지 명령 목록이 서로 모순되지 않는지 확인했다.

### 8. 검증 명령

```bash
git grep -n -e '^<<<<<<<' -e '^>>>>>>>' -- docs/CONTRIBUTING.md
git show 9e1d89b:docs/CONTRIBUTING.md | grep -E '^## '
git diff --stat 277b14d 9e1d89b
```

검증 결과는 다음과 같았다.

- 해결되지 않은 충돌 마커가 남아 있지 않았다. 문서 본문에 보이는 마커는 충돌 예시를 설명하는 코드 블록이다.
- 섹션 목차가 1번부터 15번까지 순서대로 유지됐다.
- `main` 대비 추가된 변경은 `docs/CONTRIBUTING.md` 66줄뿐이었고, `main`의 기존 내용은 하나도 삭제되지 않았다.

### 9. 결과

- 해결 커밋: [9e1d89b](https://github.com/gitflow-practice-team/github-workflow-practice/commit/9e1d89b3892029c31c8ae75d81de94654ba3574f)
- 해결 후 PR #20은 병합 가능 상태가 됐다.
- 두 사람이 각각 작성한 협업 가이드가 하나의 문서에 순서대로 통합됐다.

### 10. 배운 점

같은 파일 끝에 내용을 덧붙이는 작업은 서로 다른 영역을 수정한 것처럼 보여도 실제로는 항상 충돌한다. 특히 번호가 붙은 문서는 텍스트 병합만으로는 부족하고 번호와 목차 순서를 함께 정리해야 한다.

다음부터 문서에 새 섹션을 추가할 때는 작업 시작 전에 최신 `main`을 반영하고, 누가 어떤 번호 구간을 사용할지 미리 정한다.
